"""
Скрипт для исправления изображения "Смешанные тюльпаны"
"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                # Ищем изображение со смешанными цветами тюльпанов
                for photo in data["results"]:
                    ...
                    # Проверяем, что это смешанные тюльпаны (не только красные)
                    if (
                        "mixed" in all_text
                        or "multicolor" in all_text
                        or "various" in all_text
                    ):
                        image_url = photo.get("urls", {}).get("regular")
                        logger.info("  ✓ Найдено изображение смешанных тюльпанов")
                        return image_url

                # Если не нашли специально смешанные, берем первое
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'СМЕШАННЫЕ ТЮЛЬПАНЫ'")
    logger.info("=" * 80)

    # Ищем букет "Смешанные тюльпаны"
    flowers = Flower.objects.filter(name__icontains="Смешанные тюльпаны", in_stock=True)

    if not flowers.exists():
        logger.warning("⚠ Не найдено букетов 'Смешанные тюльпаны'!")
        sys.exit(0)
    ...
    for flower in flowers:
        ...
        # Используем специфичный запрос для смешанных тюльпанов
        search_query = "mixed colorful tulips bouquet various colors"
