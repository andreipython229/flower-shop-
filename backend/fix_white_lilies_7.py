...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые лилии (7 шт)' - ТОЛЬКО БЕЛЫЕ ЛИЛИИ")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые лилии") & Q(name__icontains="7 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые лилии (7 шт)' не найден!")
        sys.exit(1)
    ...
    search_queries = [
        "white lilies bouquet only white",
        "white lily flowers bouquet",
        "pure white lilies",
    ]
            ...
            if response.status_code == 200:
                ...
                if data.get("results") and len(data["results"]) > 0:
                    ...
                    for photo in data["results"]:
                        ...
                        # СИЛЬНЫЙ штраф за другие цвета (фиолетовый, синий)
                        ...
                        # Штраф за другие предметы на изображении (конверты, камеры)
                        if (
                    ...
                    if best_photo and best_score >= 100:  # Высокий порог
    ...
    if not image_url:
        logger.error("✗ Не удалось найти изображение белых лилий")
        sys.exit(1)
