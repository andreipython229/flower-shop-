"""Исправление изображения 'Белые розы (35 шт)' - ТОЛЬКО белые розы, без розовых"""
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in data["results"]:
                    ...
                    if "white" in all_text:
                        score += 50  # Очень большой бонус
                    if "cream" in all_text or "ivory" in all_text:
                        score += 25

                    # СИЛЬНЫЙ штраф за розовый цвет (смешанные розы)
                    if "pink" in all_text:
                        score -= 150  # Огромный штраф за розовый цвет
                ...
                if best_photo and best_score >= 50:  # Минимальный порог
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые розы (35 шт)' - ТОЛЬКО БЕЛЫЕ РОЗЫ, БЕЗ РОЗОВЫХ")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые розы") & Q(name__icontains="35 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые розы (35 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Очень специфичный запрос для ТОЛЬКО белых роз
    search_query = "pure white roses bouquet only white"
    ...
    if not image_url:
        ...
        search_query = "white roses bouquet no pink"
        logger.info(f"Поиск: '{search_query}'")
        image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY)
