"""Скрипт для копирования изображения с 'Красные гвоздики (15 шт)'
на 'Красные гвоздики (25 шт)'"""
...
if __name__ == "__main__":
    logger.info("=" * 80)
    logger.info(
        "КОПИРОВАНИЕ ИЗОБРАЖЕНИЯ С 'Красные гвоздики (15 шт)' "
        "НА 'Красные гвоздики (25 шт)'"
    )
    logger.info("=" * 80)
    ...
    target_flower = Flower.objects.filter(
        Q(name__icontains="Красные гвоздики") & Q(name__icontains="25 шт")
    ).first()

    if not target_flower:
        logger.error("✗ Цветок 'Красные гвоздики (25 шт)' не найден!")
        sys.exit(1)
