"""Исправление изображения 'Оранжевые розы (11 шт)' для картинки 127"""
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in results:
                    ...
                    # Бонус за упоминание роз
                    if "rose" in all_text or "roses" in all_text:
                        score += 50

                    # ОБЯЗАТЕЛЬНО должен быть оранжевый цвет
                    if "orange" in all_text:
                        score += 50
                    if "tangerine" in all_text:
                        score += 30
                    if "coral" in all_text and "orange" in all_text:
                        score += 25

                    # СИЛЬНЫЙ штраф за другие цвета
                    if "yellow" in all_text and "orange" not in all_text:
                        score -= 200
                    if "red" in all_text and "orange" not in all_text:
                        score -= 200
                    if "pink" in all_text and "orange" not in all_text:
                        score -= 200
                    if "white" in all_text and "orange" not in all_text:
                        score -= 150
                    if "purple" in all_text:
                        score -= 200
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Оранжевые розы (11 шт)' ДЛЯ КАРТИНКИ 127")
    logger.info("=" * 80)

    # Находим "Оранжевые розы (11 шт)" - картинка 127
    flower = Flower.objects.filter(
        Q(name__icontains="Оранжевые розы") & Q(name__icontains="11 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Оранжевые розы (11 шт)' не найден!")
        sys.exit(1)
