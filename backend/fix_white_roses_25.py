"""Исправление изображения 'Белые розы (25 шт)' через Unsplash API с очень строгим фильтром"""
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in data["results"]:
                    ...
                    if "pink" in all_text and "white" not in all_text:
                        score -= 100  # Огромный штраф за розовый без белого
                ...
                if (
                    best_photo and best_score >= 50
                ):  # Минимальный порог - должны быть розы И белый цвет
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые розы (25 шт)' С ОЧЕНЬ СТРОГИМ ФИЛЬТРОМ")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые розы") & Q(name__icontains="25 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые розы (25 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Очень специфичный запрос для белых роз
