"""Исправление изображения 'Розовые эустомы (9 шт)' через Unsplash API"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Розовые эустомы (9 шт)'")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Розовые эустомы") & Q(name__icontains="9 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Розовые эустомы (9 шт)' не найден!")
        sys.exit(1)
    ...
    search_queries = [
        "pink eustoma lisianthus bouquet",
        "pink lisianthus flowers",
        "pink eustoma flowers",
    ]
            ...
            if response.status_code == 200:
                ...
                if data.get("results") and len(data["results"]) > 0:
                    ...
                    for photo in data["results"]:
                        ...
                        # ОБЯЗАТЕЛЬНО должны быть эустомы/лизиантусы
                        if "eustoma" in all_text or "lisianthus" in all_text:
                            score += 100
                        else:
                            continue
                        ...
                        if "rose" in all_text or "roses" in all_text:
                            score -= 200
                        if "carnation" in all_text or "lily" in all_text:
                            score -= 150
    ...
    if not image_url:
        logger.error("✗ Не удалось найти изображение розовых эустом")
        sys.exit(1)
