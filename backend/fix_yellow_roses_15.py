"""Исправление изображения 'Желтые розы (15 шт)' через Unsplash API"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Желтые розы (15 шт)'")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Желтые розы") & Q(name__icontains="15 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Желтые розы (15 шт)' не найден!")
        sys.exit(1)
            ...
            if response.status_code == 200:
                ...
                if data.get("results") and len(data["results"]) > 0:
                    ...
                    for photo in data["results"]:
                        ...
                        # Штраф за подарки/коробки
                        if (
                            "gift" in all_text
                            or "box" in all_text
                            or "present" in all_text
                        ):
                            score -= 200
    ...
    if not image_url:
        logger.error("✗ Не удалось найти изображение желтых роз")
        sys.exit(1)
