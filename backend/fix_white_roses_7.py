"""Исправление изображения 'Белые розы (7 шт)' через Unsplash API
- другое изображение, чем на картинках 133 и 134"""
...
def get_unsplash_image(search_query, api_key, skip_index=0):
    """Получает изображение из Unsplash API с ОЧЕНЬ строгим фильтром для белых роз"""
    if not api_key:
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in results:
                    ...
                    # СИЛЬНЫЙ штраф за неправильные цвета
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info(
        "ИСПРАВЛЕНИЕ 'Белые розы (7 шт)' - ДРУГОЕ ИЗОБРАЖЕНИЕ, "
        "ЧЕМ НА КАРТИНКАХ 133 И 134"
    )
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые розы") & Q(name__icontains="7 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые розы (7 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Используем skip_index=15 чтобы получить ТРЕТЬЕ уникальное изображение
    # (отличное от 133 и 134)
    # 133 использует skip_index=0, 134 использует skip_index=5,
    # 135 использует skip_index=15
    search_query = "white roses bouquet"
    logger.info(
        f"Поиск: '{search_query}' "
        f"(пропуск первых 15 результатов для получения УНИКАЛЬНОГО изображения)"
    )

    image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY, skip_index=15)

    if not image_url:
        logger.warning("⚠ Попробуем альтернативный запрос...")
        search_query = "white roses flowers"
        logger.info(f"Поиск: '{search_query}' (пропуск первых 15 результатов)")
        image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY, skip_index=15)
