"""Исправление изображения 'Белые лилии' для картинки 74 - отличное от картинки 73"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые лилии' ДЛЯ КАРТИНКИ 74 - ОТЛИЧНОЕ ОТ КАРТИНКИ 73")
    logger.info("=" * 80)

    # Находим "Белые лилии (5 шт)" - картинка 74
    flower = Flower.objects.filter(
        Q(name__icontains="Белые лилии") & Q(name__icontains="5 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые лилии (5 шт)' не найден!")
        sys.exit(1)
    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Используем skip_index=5 чтобы получить ДРУГОЕ изображение, чем на картинке 73
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                # Пропускаем первые 3 результата для разнообразия (меньше, чтобы было
                # больше вариантов)
                results = (
                    data["results"][3:] if len(data["results"]) > 3 else data["results"]
                )
                ...
                ):  # Базовый score = 50, даже без метаданных
