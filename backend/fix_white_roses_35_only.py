"""Исправление изображения 'Белые розы (35 шт)' - ТОЛЬКО нормальные белые розы"""
...
def get_unsplash_image(search_query, api_key):
    """Получает изображение из Unsplash API - ТОЛЬКО нормальные белые розы"""
    if not api_key:
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in data["results"]:
                    ...
                    # СИЛЬНЫЙ штраф за сушеные розы
                    if "dried" in all_text or "dry" in all_text:
                        score -= 200  # Огромный штраф за сушеные розы

                    # СИЛЬНЫЙ штраф за розовый цвет
                    if "pink" in all_text:
                        score -= 150
                    ...
                    # Штраф за записки/текст на изображении
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые розы (35 шт)' - НОРМАЛЬНЫЕ БЕЛЫЕ РОЗЫ")
    logger.info("=" * 80)
    ...
    # Запрос для нормальных белых роз
    search_query = "fresh white roses bouquet"
