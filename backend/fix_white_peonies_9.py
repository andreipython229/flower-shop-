"""Исправление изображения 'Белые пионы (9 шт)' - ТОЛЬКО белые пионы, без других цветов"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые пионы (9 шт)' - ТОЛЬКО БЕЛЫЕ ПИОНЫ")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые пионы") & Q(name__icontains="9 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые пионы (9 шт)' не найден!")
        sys.exit(1)
    ...
    search_queries = [
        "white peonies bouquet only white",
        "pure white peonies bouquet",
        "white peony flowers",
    ]
            ...
            if response.status_code == 200:
                ...
                if data.get("results") and len(data["results"]) > 0:
                    ...
                    for photo in data["results"]:
                        ...
                        # СИЛЬНЫЙ штраф за другие цвета (коралловый, розовый)
    ...
    if not image_url:
        logger.error("✗ Не удалось найти изображение белых пионов")
        sys.exit(1)
