"""Исправление изображения 'Двухцветные розы (11 шт)' через Unsplash API"""
...
def get_unsplash_image(search_query, api_key):
    """Получает изображение из Unsplash API с строгим фильтром для двухцветных роз"""
    if not api_key:
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in data["results"]:
                    ...
                    # Бонус за двухцветность
                    if (
                        "two-tone" in all_text
                        or "two tone" in all_text
                        or "bicolor" in all_text
                        or "bi-color" in all_text
                    ):
                        score += 25
                    if "multicolor" in all_text or "multi-color" in all_text:
                        score += 15
                    if "variegated" in all_text:
                        score += 10

                    # Бонус за упоминание двух цветов одновременно
                    color_count = 0
                    colors = [
                        "red",
                        "pink",
                    ...
                    for color in colors:
                        if color in all_text:
                            color_count += 1
                    if color_count >= 2:
                        score += 15  # Бонус за упоминание нескольких цветов
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Двухцветные розы (11 шт)' ЧЕРЕЗ UNSPLASH API")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Двухцветные розы") & Q(name__icontains="11 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Двухцветные розы (11 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Специфичный запрос для двухцветных роз
    search_query = "two-tone roses bouquet bicolor"
    ...
    if not image_url:
        ...
        search_query = "multicolor roses bouquet variegated"
        logger.info(f"Поиск: '{search_query}'")
        image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY)
