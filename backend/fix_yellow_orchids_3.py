"""Исправление изображения 'Желтые орхидеи (3 шт)' через Unsplash API"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Желтые орхидеи (3 шт)'")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Желтые орхидеи") & Q(name__icontains="3 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Желтые орхидеи (3 шт)' не найден!")
        sys.exit(1)
    ...
    search_queries = [
        "yellow orchid phalaenopsis bouquet",
        "yellow orchids flowers",
            ...
            if response.status_code == 200:
                ...
                if data.get("results") and len(data["results"]) > 0:
                    ...
                    for photo in data["results"]:
                        ...
                        # ОБЯЗАТЕЛЬНО должны быть орхидеи
                        if (
                            "orchid" in all_text
                            or "orchids" in all_text
                            or "phalaenopsis" in all_text
                        ):
                            score += 100
                        else:
                            continue
                        ...
                        if "white" in all_text and "yellow" not in all_text:
                            score -= 200
                        if "pink" in all_text and "yellow" not in all_text:
                            score -= 200
                        if "purple" in all_text and "yellow" not in all_text:
                            score -= 200
    ...
    if not image_url:
        logger.error("✗ Не удалось найти изображение желтых орхидей")
        sys.exit(1)
