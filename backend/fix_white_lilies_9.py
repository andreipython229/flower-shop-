...
def get_unsplash_image(search_query, api_key):
    """Получает изображение из Unsplash API - ТОЛЬКО белые лилии"""
    if not api_key:
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in data["results"]:
                    ...
                    # ОБЯЗАТЕЛЬНО должны быть лилии
                    if "lily" not in all_text and "lilies" not in all_text:
                        continue

                    # Большой бонус за "lily"
                    ...
                    # СИЛЬНЫЙ штраф за другие цвета (фиолетовые, синие и т.д.)
                    if "purple" in all_text or "violet" in all_text:
                        score -= 150  # Огромный штраф за фиолетовый
                    if "blue" in all_text:
                        score -= 150
                    ...
                    if "mixed" in all_text or "bouquet" in all_text:
                        # Если есть "mixed", но нет "white lily", то штраф
                        if "white" not in all_text or "lily" not in all_text:
                            score -= 100

                    # Штраф за неправильные цветы
                    if (
                        "rose" in all_text
                        or "carnation" in all_text
                        or "tulip" in all_text
                    ):
                        score -= 100

                    # Штраф за другие предметы на изображении (конверты, камеры и т.д.)
                    if (
                ...
                if (
                    best_photo and best_score >= 20
                ):  # Снижаем порог для более гибкого поиска
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Белые лилии (9 шт)' - ТОЛЬКО БЕЛЫЕ ЛИЛИИ")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые лилии") & Q(name__icontains="9 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые лилии (9 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Запрос для белых лилий
    ...
    if not image_url:
        ...
        search_query = "white lily flowers"
        logger.info(f"Поиск: '{search_query}'")
        image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY)
