"""Исправление изображения 'Розовые герберы (21 шт)' через Unsplash API"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Розовые герберы (21 шт)'")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Розовые герберы") & Q(name__icontains="21 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Розовые герберы (21 шт)' не найден!")
        sys.exit(1)
            ...
            if response.status_code == 200:
                ...
                if data.get("results") and len(data["results"]) > 0:
                    ...
                    for photo in data["results"]:
                        ...
                        # ОБЯЗАТЕЛЬНО должно быть "gerbera"
                        if "gerbera" in all_text:
                            score += 100
                        else:
                            continue
    ...
    if not image_url:
        logger.error("✗ Не удалось найти изображение розовых гербер")
        sys.exit(1)
