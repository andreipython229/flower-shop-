"""Исправление изображения 'Белые розы (11 шт)' через Unsplash API - другое изображение, чем на картинке 133"""
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info(
        "ИСПРАВЛЕНИЕ 'Белые розы (11 шт)' - ДРУГОЕ ИЗОБРАЖЕНИЕ, ЧЕМ НА КАРТИНКЕ 133"
    )
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Белые розы") & Q(name__icontains="11 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Белые розы (11 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Используем skip_index=5 чтобы получить ДРУГОЕ изображение, чем на картинке 133
    search_query = "white roses bouquet pure white"
    logger.info(
        f"Поиск: '{search_query}' (пропуск первых 5 результатов для разнообразия)"
    )

    image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY, skip_index=5)

    if not image_url:
        logger.warning("⚠ Попробуем альтернативный запрос...")
        search_query = "white roses flowers bouquet"
        logger.info(f"Поиск: '{search_query}' (пропуск первых 5 результатов)")
        image_url = get_unsplash_image(search_query, UNSPLASH_ACCESS_KEY, skip_index=5)
