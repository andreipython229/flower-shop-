"""Скрипт для исправления изображения 'Красные гвоздики (15 шт)' через Unsplash API"""
        ...
        if response.status_code == 200:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for photo in data["results"]:
                    ...
                    # Штраф за неправильный цвет (розовый вместо красного)
                    if "pink" in all_text and "red" not in all_text:
                        score -= 30  # Сильный штраф за розовый цвет при поиске красного
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("ИСПРАВЛЕНИЕ 'Красные гвоздики (15 шт)' ЧЕРЕЗ UNSPLASH API")
    logger.info("=" * 80)

    # Находим цветок
    flower = Flower.objects.filter(
        Q(name__icontains="Красные гвоздики") & Q(name__icontains="15 шт")
    ).first()

    if not flower:
        logger.error("✗ Цветок 'Красные гвоздики (15 шт)' не найден!")
        sys.exit(1)

    logger.info(f"Найден цветок: {flower.name} (ID: {flower.id})")

    # Специфичный запрос для красных гвоздик
    search_query = "red carnation flowers bouquet"
